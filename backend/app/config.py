...
class Settings:
    ...
    # Максимальная длительность одного файла (сек). Потолок задаёт Runpod: у него
    # лимит 10 MiB на тело /run, а шлюз шлёт аудио инлайн в base64. Даже после
    # сжатия до 16 kHz mono (32 КБ/с) 10 MiB base64 ≈ 4 минуты аудио. Длиннее —
    # нужна нарезка на части (см. docs/tasks/07-scaling.md), пока не реализовано.
    max_audio_seconds: int = _get_int("MAX_AUDIO_SECONDS", 60 * 4)  # 4 минуты
