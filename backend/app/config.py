...
class Settings:
    ...
    # Штатные -1.0 подобраны под английский; на казахском модель в среднем
    # менее уверена, и порог выбрасывал настоящие реплики — отсюда «пропущенные
    # предложения». -1.5 оставляет отсев мусора, но не режет живую речь.
