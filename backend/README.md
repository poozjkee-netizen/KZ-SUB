...
> **Нарезка субтитров живёт в воркере.** Правки `segmentation.py` и режим
> `style` из запроса требуют пересборки GPU-образа — `fly deploy` их не доносит.
> Быстрая проверка, что на шлюзе новый код: в `GET /health` есть поле `styles`.
