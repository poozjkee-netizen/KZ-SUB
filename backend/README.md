...
| `KZSUB_BOT_USERS_DB`          | (пусто)      | путь к БД языковых предпочтений бота (пусто = `STATE_DIR/bot_users.db`) |
| `KZSUB_LANDING_URL`           | адрес Pages  | ссылка на лендинг в приветствии и `/buy` бота |
| `KZSUB_INSTALLER_URL`         | адрес Pages  | прямая ссылка на установщик; бот шлёт файл по ней (`/download`) |
...
**Первое сообщение от нового пользователя — только выбор языка** (қазақша/русский,
инлайн-кнопки), прежде чем бот скажет что-либо ещё. Выбор хранится по
Telegram-user_id в отдельной БД (`app/bot_users.py`) — отдельно от лицензий,
потому что язык нужен ещё до того, как у человека вообще появится ключ.
Дальше все ответы этому пользователю идут на выбранном языке; сменить — `/lang`
в любой момент. Продавцу (заявки на подтверждение оплаты) бот всегда пишет
по-русски — это его рабочий интерфейс, а не клиентский, локализовать незачем.

Demo (3 мин) выдаётся автоматически, один раз на Telegram-аккаунт. Под сообщением
с ключом две кнопки: сначала «Скачать плагин» (ключ бесполезен, пока панель не
установлена), затем переход к Standard — чтобы демо не заканчивалось тупиком,
когда минуты кончатся. Команда `/download` шлёт установщик файлом прямо в чат:
Telegram забирает его по `KZSUB_INSTALLER_URL`, поэтому копия рядом со шлюзом не
нужна; если хостинг недоступен, бот присылает ссылку. Standard (60 мин/30 дней) — полу-авто: клиент жмёт «Я оплатил»
после перевода на Kaspi, заявка с кнопками Подтвердить/Отклонить уходит продавцу
(`KZSUB_TELEGRAM_ADMIN_IDS`); после подтверждения бот сам создаёт лицензию и
присылает ключ клиенту. Kaspi не даёт публичного API для авто-проверки перевода,
поэтому решение — за человеком.
...
fly ssh console -C "python -m app.events funnel --days 30"
fly ssh console -C "python -m app.events recent --limit 20"
```

### Воронка (`funnel`)
Прогоны — это последний шаг пути. Если их мало, по ним не понять, потеряли мы
человека на установке, на ключе или он вообще не дошёл до бота. Поэтому бот
отмечает шаги: открыл бота → выбрал язык → взял демо-ключ → скачал установщик →
открыл покупку → нажал «оплатил» → купил.

Шаг **«сделал субтитры»** не отмечается ботом — его нельзя знать со стороны
Telegram. Он выводится соединением воронки с прогонами по хешу ключа: выдали
ключ и по нему был успешный прогон — значит, человек дошёл до результата.
Окно (`--days`) применяется к шагу, а не к прогону: вчерашняя регистрация не
считается провалом только потому, что ролик ещё не смонтирован.

Доли считаются **от вошедших**, а не от предыдущего шага: путь не строго
вложенный (покупку можно открыть, не взяв демо), и «от предыдущего» давало бы
доли больше 100%. Строка «крупнейший обрыв» показывает, между какими соседними
шагами теряется больше всего людей — туда и прикладывать силы.

**Источники.** Кнопки лендинга ведут на `t.me/np_subbot?start=<метка>`
(`dock`, `final`, `price_demo`, `price_buy`, `footer`); Telegram доставляет это
боту как `/start dock`, и в отчёте видно, какая кнопка приводит людей. Сам
лендинг лежит на GitHub Pages и своих логов не даёт — это единственный способ
померить приток. `direct` = пришёл к боту без метки.

Про приватность воронки без прикрас: Telegram ID — короткое число, множество
таких чисел перебирается за секунды, поэтому его хеш защищает не от того, у
кого уже есть эта БД, а от случайного взгляда и от утечки списка ID при
выгрузке. Он нужен, чтобы считать людей, а не чтобы обещать анонимность.
...
Учёт не может уронить прогон или выдачу ключа: `record_run`/`record_step`
глотают свои ошибки, а пропавшую таблицу пересоздают на месте. Выключается
целиком через `KZSUB_ANALYTICS=false`.
